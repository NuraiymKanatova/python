...
def random_cell(exclude: set, margin=1) -> tuple:
    """Return a random (col, row) not in exclude and not near arena borders."""
    while True:
        c = random.randint(margin, COLS - 1 - margin)
        r = random.randint(margin, ROWS - 1 - margin)
...
def cell_to_px(cell):
    return (
        ARENA_X + cell[0] * CELL_SIZE,
        ARENA_Y + cell[1] * CELL_SIZE
    )
...
class SnakeGame:
    ...
    def _draw(self, now):
        ...
        pygame.draw.rect(self.screen, (0,0,0), (ARENA_X, ARENA_Y, ARENA_WIDTH, ARENA_HEIGHT))
        pygame.draw.rect(self.screen, GREEN, (ARENA_X, ARENA_Y, ARENA_WIDTH, ARENA_HEIGHT), 3)

        # Grid
        if self.settings.get("grid", False):
            for c in range(COLS + 1):
                x = ARENA_X + c * CELL_SIZE
                pygame.draw.line(self.screen, (60,60,60), (x, ARENA_Y), (x, ARENA_Y + ARENA_HEIGHT))

            for r in range(ROWS + 1):
                y = ARENA_Y + r * CELL_SIZE
                pygame.draw.line(self.screen, (60,60,60), (ARENA_X, y), (ARENA_X + ARENA_WIDTH, y))

        # Obstacles
        for cell in self.obstacles:
            x, y = cell_to_px(cell)
            pygame.draw.rect(self.screen, (90, 90, 90), (x+2, y+2, CELL_SIZE-4, CELL_SIZE-4), border_radius=4)
            pygame.draw.rect(self.screen, (150,150,150), (x+2, y+2, CELL_SIZE-4, CELL_SIZE-4), 2, border_radius=4)
        ...
        # Snake — clean rounded style, no ugly image
        snake_color = tuple(self.settings.get("snake_color", [100, 220, 100]))
        head_color = tuple(min(255, c + 35) for c in snake_color)
        ...
        for i, seg in enumerate(self.snake):
            ...
            if i == 0:
                pygame.draw.rect(self.screen, head_color, rect, border_radius=7)
    ...
    def _draw_hud(self, now):
        ...
        pygame.draw.rect(self.screen, (10, 10, 20), (0, 0, HUD_WIDTH, HEIGHT))
        pygame.draw.line(self.screen, GREEN, (HUD_WIDTH, 0), (HUD_WIDTH, HEIGHT), 2)
        
        title = pygame.font.SysFont("Arial", 28, bold=True)
        sub   = pygame.font.SysFont("Arial", 16)

        t1 = title.render("SNAKE", True, GREEN)
        t2 = sub.render("TSIS 4", True, GRAY)

        self.screen.blit(t1, (35, 25))
        self.screen.blit(t2, (50, 60))
        pygame.draw.line(self.screen, GRAY, (20, 90), (HUD_WIDTH - 20, 90), 1)
        ...
        self.screen.blit(info, (ARENA_X + ARENA_WIDTH - info.get_width() - 10, HEIGHT - 25))
    # ── Main loop ─────────────────────────────────────────────────────────────
